...
static void _MDPointSourceRychtecka (int itemID) {
 ...
 //       WWTP_Removal2 = WWTP_Removal * (1.0 - deterioration) > 0.9999 ? 0.9999 : WWTP_Removal * (1.0 - deterioration);    // This is for individual runs of deterioration (for Sensitivity runs, NOT MONTECARLO)
          ...
          WWTP_Removal2 = WWTP_Removal + deterioration < 0.0001 ? 0.0001 : WWTP_Removal + deterioration;                    // This is for MonteCarlo that uses standard deviations around mean
...
//	WWTP_Removal = WWTP_Treatment == 1.0 ? 0.10 : WWTP_Removal;		// Primary treatment
...
//	WWTP_Removal = WWTP_Treatment == 5.0 ? 0.90 : WWTP_Removal;		// Community Amenities
 	...
 	WWTP_Removal = WWTP_Treatment == 1.0 ? 0.10 : WWTP_Removal;		// Primary treatment
	WWTP_Removal = WWTP_Treatment == 2.0 ? 0.50 : WWTP_Removal;		// Secondary treatment - Long Island Sound TMDL, 19.6 mg/L effluent (40 mg/L influent concentration)
	WWTP_Removal = WWTP_Treatment == 3.0 ? 0.80 : WWTP_Removal;		// Advanced treatment I - Van Drecht, 8 mg/L effluent
	WWTP_Removal = WWTP_Treatment == 4.0 ? 0.80 : WWTP_Removal;		// Advanced treatment II
	WWTP_Removal = WWTP_Treatment == 5.0 ? 0.90 : WWTP_Removal;		// Community Amenities
        ...
        WWTP_Removal2 = WWTP_Removal + deterioration > 0.9999 ? 0.9999 : WWTP_Removal + deterioration;                        // This is for individual runs of deterioration (NOT MONTECARLO)
        WWTP_Removal2 = WWTP_Removal2 <= 0.0 ? 0.0 : WWTP_Removal2;                                         // RJS 101515
...
//        WWTP_InKgPerDay = (WWTP_KgTNperPerson * WWTP_PopServed) * (0.485 + ((WWTP_Removal2 / WWTP_MaxRemoval) * 0.255));    // Dumont et al. 2005 for calculation of DIN from TN       
        WWTP_InKgPerDay = (WWTP_KgTNperPerson * WWTP_PopServed);    	// all organic N nitrifies to NO3 quickly    3/2/2016
